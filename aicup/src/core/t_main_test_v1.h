...
void add_some_by_random(vector<t_global_user>& garr, t_season& season) {
    ...
    if(rand()%2==0){for(int i=0;i<4;i++)((uint64_t*)&ai[0])[i]=rnd();}//"bin/" + to_string(new_uid) + "_v0.bin";
...
std::vector<double> simulate_arena(const std::vector<uint64_t>& strategies) {
  return simulate_arena_8byte((std::vector<Strategy>&)strategies);
}
std::vector<double> simulate_arena_8byte(const std::vector<Strategy>& strategies) {
...
void add_finished_game(const t_main_test&mt,t_season& season) {
    ...
    for (size_t i = 0; i < players.size(); ++i) {
      ais[i]=((uint64_t*)&season.uid2scoder[players[i]].sarr.back().cdn_bin_url[0])[R];//mt.garr[players[i]]
    }
...
int mt_games_per_phase=4000;//2*64000;
...
void main_test(){
  ...
  for(auto v:{3767069945764529280ull,evolved_defect_num,smart_defect_num,640ull,896ull,736ull,6948492336081867392ull,4985045709669101440ull,7463340112207481728ull,
