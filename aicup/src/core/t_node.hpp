...
struct t_node:t_node_cache{
  ...
  bool new_game(const t_game_decl&gd){
    ...
    g.slot2api.reserve(gd.arr.size());
  ...
  struct t_event_loop_v2{
    ...
    void wait_for_socket(const std::string& socket_path, int max_wait_ms=1024*5, int poll_interval_ms=16) {
  ...
  int main(const string&token){
    ...
    swd=make_unique<SocketWithDecoder>(0?ovbox_main_ip_port:local_main_ip_port,std::move(cb),true);
    swd->begin();
